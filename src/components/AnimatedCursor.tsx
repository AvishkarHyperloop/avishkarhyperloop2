...
export default function AnimatedCursor() {
  ...
  useEffect(() => {
    ...
    const onMouseOver = (e: MouseEvent) => {
      ...
      if (cursorRingRef.current) {
        if (isLink) {
          cursorRingRef.current.classList.add("scale-150", "bg-green-500/10");
          cursorRingRef.current.classList.remove("scale-100");
        } else {
          cursorRingRef.current.classList.remove("scale-150", "bg-green-500/10");
  ...
  return (
    <>
      ...
      <div
        ref={cursorDotRef}
        className="fixed top-0 left-0 w-2 h-2 bg-green-500 rounded-full pointer-events-none z-[9999] mix-blend-difference will-change-transform"
        style={{ transform: "translate3d(-100px, -100px, 0)" }}
      />
      {/* Large circle - lags behind slightly */}
      <div
        ref={cursorRingRef}
        className="fixed top-0 left-0 w-8 h-8 rounded-full border-2 border-green-500 pointer-events-none z-[9999] mix-blend-difference will-change-transform transition-transform duration-200 ease-out scale-100"
        style={{ transform: "translate3d(-100px, -100px, 0)" }}
      />
    </>
  );
